module Page.Post exposing (Model, Msg, init, subscriptions, toSession, update, view)
...
import Log
import Page
import Post exposing (Full, Post, Preview)
import Post.Body exposing (Body)
import Post.Comment as Comment exposing (Comment)
                ...
                div [ class "article-page" ]
                            ...
                            , div [ class "article-meta" ] <|
                                List.append
                                    ...
                                    , div [ class "info" ]
                                        ...
                                        , Timestamp.view model.timeZone (Post.post article).created_at
                                        ]
                                    ]
                                    buttons
                    ...
                    , div [ class "container page" ]
                        [ div [ class "row article-content" ]
                            [ div [ class "col-md-12" ]
                                [ Post.Body.toHtml (Post.body article) [] ]
                            ]
                        , hr [] []
...
viewButtons : Cred -> Post Full -> Author -> List (Html Msg)
viewButtons cred article author =
    case author of
...
type Msg
    ...
    | CompletedLoadArticle (Result Http.Error (Post Full))
    ...
    | CompletedFavoriteChange (Result Http.Error (Post Full))
...
update msg model =
    case msg of
        ...
        ClickedFavorite cred slug body ->
            ( model, fave Post.favorite cred slug body )

        ClickedUnfavorite cred slug body ->
            ( model, fave Post.unfavorite cred slug body )
        ...
        CompletedFollowChange (Ok newAuthor) ->
            case model.article of
                Loaded article ->
                    --  | article = Loaded (Post.mapAuthor (\_ -> newAuthor) article)
                    ( model, Cmd.none )
...
fave : (Slug -> Cred -> Http.Request (Post Preview)) -> Cred -> Slug -> Body -> Cmd Msg
fave toRequest cred slug body =
    toRequest slug cred
        |> Http.toTask
        |> Task.map (Post.fromPreview body)
...
favoriteButton : Cred -> Post Full -> Html Msg
favoriteButton cred article =
    div [] []
...
-- in
-- if favorited then
--     Post.unfavoriteButton cred (ClickedUnfavorite cred slug body) [] kids
-- else
--     Post.favoriteButton cred (ClickedFavorite cred slug body) [] kids


deleteButton : Cred -> Post a -> Html Msg
deleteButton cred article =
    let
        msg =
            ClickedDeleteArticle cred (Post.slug article)
    in
    button [ class "btn btn-outline-danger btn-sm", onClick msg ]
        [ i [ class "ion-trash-a" ] [], text " Delete Article" ]


editButton : Post a -> Html Msg
editButton article =
    a [ class "btn btn-outline-secondary btn-sm", Route.href (Route.EditArticle (Post.slug article)) ]
