module Page.Post.Editor exposing (Model, Msg, initEdit, initNew, subscriptions, toSession, update, view)
...
initEdit session slug =
    ...
    , Cmd.batch
        [ Post.fetch (Session.cred session) slug
            |> Http.toTask
...
view model =
    ...
    , content =
        case Session.cred model.session of
            ...
            Nothing ->
                text "Sign in to edit this Post."
    }
...
type Msg
    ...
    | EnteredTags String
    | EnteredTitle String
    | CompletedCreate (Result Http.Error (Post Full))
    | CompletedEdit (Result Http.Error (Post Full))
    | CompletedArticleLoad (Result ( Slug, Http.Error ) (Post Full))
    | GotSession Session
...
update msg model =
    case msg of
        ...
        CompletedCreate (Ok article) ->
            ( model
            , Route.Article (Post.slug article)
        ...
        CompletedArticleLoad (Ok article) ->
            let
                ...
                status =
                    Editing (Post.slug article)
                        []
                        { title = title
                        , body = Post.Body.toMarkdownString (Post.body article)
...
create : TrimmedForm -> Cred -> Http.Request (Post Full)
    ...
    let
        ...
        body =
            Encode.object [ ( "article", article ) ]
                |> Http.jsonBody
    in
    Decode.field "article" (Post.fullDecoder (Just cred))
        |> Api.post (Endpoint.posts []) (Just cred) body
...
edit : Slug -> TrimmedForm -> Cred -> Http.Request (Post Full)
